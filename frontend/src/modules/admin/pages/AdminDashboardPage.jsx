...
const ENTITY_STYLES = {
    users:   { rgb: '59, 130, 246',  hex: '#3b82f6', icon: Users },
    sites:   { rgb: '16, 185, 129',  hex: '#10b981', icon: Globe },
    reports: { rgb: '239, 68, 68',   hex: '#ef4444', icon: AlertTriangle },
    tickets: { rgb: '139, 92, 246',  hex: '#8b5cf6', icon: MessageSquare },
    default: { rgb: '107, 114, 128', hex: '#6b7280', icon: Activity }
};
...
const StatCard = ({ title, value, type, trend, trendValue, loading }) => {
    ...
    const bgOpacity = `rgba(${styleConfig.rgb}, 0.1)`;
    const iconColor = styleConfig.hex;
    const styles = {
        card: {
                ...
                {data.map((item, index) => {
                    ...
                    return (
                        <div key={index} style={{flex: 1, height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'flex-end', alignItems: 'center', position: 'relative', zIndex: 1}} className="group">
                            <div className="opacity-0 group-hover:opacity-100 transition-opacity absolute bottom-full mb-2 left-1/2 -translate-x-1/2 bg-gray-800 text-white text-[11px] py-1 px-2 rounded pointer-events-none whitespace-nowrap z-20 shadow-lg">
    ...
    return (
        <div style={{ 
            ...
            <div style={{
                ...
                background: `rgba(${styleConfig.rgb}, 0.1)`, 
                color: styleConfig.hex,
                flexShrink: 0
            }}>
                <Icon size={18} />
            </div>
