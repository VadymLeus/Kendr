...
import { Mail, Calendar, Layout, AlertTriangle, Smartphone, Hash, Globe, CheckCircle, Ban } from 'lucide-react';
        ...
        <BaseDetailsPanel title="Деталі користувача" onClose={onClose} onDelete={() => onDelete(user.id)} deleteLabel="Видалити повністю">
            ...
            <div style={styles.hero}>
                <div style={styles.avatarWrapper} onClick={handleVisitProfile} 
                     ...
                     onMouseLeave={e => { if(user.status !== 'suspended') e.currentTarget.style.transform = 'scale(1)' }}
                >
                ...
                <div onClick={handleVisitProfile} 
                     style={{fontSize: '22px', fontWeight: 'bold', marginBottom: '4px', cursor: user.status === 'suspended' ? 'default' : 'pointer', color: user.status === 'suspended' ? 'var(--platform-danger)' : 'var(--platform-text-primary)', transition: 'color 0.2s'}}
                     onMouseEnter={e => { if(user.status !== 'suspended') e.currentTarget.style.color = 'var(--platform-accent)' }}
                     onMouseLeave={e => { if(user.status !== 'suspended') e.currentTarget.style.color = 'var(--platform-text-primary)' }}
                >
                    {user.username}
                </div>
            </div>
