...
const StatusButton = ({ statusKey, currentStatus, onClick }) => {
    ...
    return (
        <button
            ...
            className={`
                flex-1 px-1 py-2 rounded-md border cursor-pointer text-xs font-semibold transition-all duration-200 text-center
    ...
    return (
        <div className="w-full h-full flex flex-col box-border px-1">
            <div className="bg-(--platform-card-bg) rounded-xl border border-(--platform-border-color) shadow-sm h-full flex overflow-hidden">
                <div className="w-1/3 min-w-70 max-w-105 border-r border-(--platform-border-color) flex flex-col bg-(--platform-card-bg) shrink-0">
                    <div className="min-h-17.5 px-3 lg:px-4 border-b border-[#2d3748] bg-[#1a202c] flex items-center gap-2 lg:gap-3 text-white shrink-0">
                        <Input 
                            placeholder="Пошук..." 
                            value={searchTerm}
                            ...
                            wrapperStyle={{margin: 0, flex: 1, minWidth: 0}}
                        />
                                ...
                                return (
                                    ...
                                    >
                                        <div className="flex justify-between items-start mb-1 gap-2">
                                            <div className="font-semibold text-sm text-(--platform-text-primary) truncate flex-1 min-w-0">
                                                {sub.form_data.name || 'Без імені'}
                                            </div>
                                            <button 
                                                onClick={(e) => handleTogglePin(sub.id, e)}
                                                className={`
                                                    bg-transparent border-none cursor-pointer p-0.5 flex transition-colors shrink-0
                                        ...
                                        <div className="flex justify-between items-center gap-2">
                                            <span 
                                                className="text-[0.7rem] px-2 py-0.5 rounded border font-semibold inline-flex items-center gap-1 shrink-0"
                                                style={{
                                            ...
                                            <span className="text-[0.7rem] text-(--platform-text-secondary) shrink-0">
                ...
                <div className="flex-1 flex flex-col overflow-hidden bg-[#1a202c] min-w-0">
                    {selectedSubmission ? (
                            ...
                            <div className="px-4 lg:px-6 border-b border-[#2d3748] flex justify-between items-center bg-[#1a202c] min-h-17.5 shrink-0 text-white gap-4">
                                <div className="flex-1 min-w-0">
                                    ...
                                    <div className="flex items-center gap-2.5 text-[#a0aec0] text-xs truncate">
                                        ...
                                        <span className="shrink-0">#{selectedSubmission.id}</span>
                                    </div>
                                </div>
                                <div className="flex gap-2 shrink-0">
                                    ...
                                    <DangerButton onClick={() => handleDelete(selectedSubmission.id)}>
                                        <Trash size={16} /> <span className="hidden sm:inline">Видалити</span>
                                    </DangerButton>
                                </div>
                            </div>
                            <div className="custom-scrollbar p-4 lg:p-6 overflow-y-auto flex-1">
                                <div className="grid grid-cols-1 xl:grid-cols-2 gap-4 mb-5">
                                    <div className="bg-(--platform-card-bg) p-4 rounded-lg border border-(--platform-border-color) shadow-sm min-w-0">
                                        <div className="text-xs uppercase text-(--platform-text-secondary) font-bold mb-2.5 flex items-center gap-1.5">
                                            <User size={14} className="shrink-0"/> Від кого
                                        </div>
                                        <div className="font-semibold text-base mb-1 text-(--platform-text-primary) truncate">
                                            {selectedSubmission.form_data.name}
                                        </div>
                                        <div 
                                            onClick={() => copyToClipboard(selectedSubmission.form_data.email)}
                                            className="text-(--platform-accent) cursor-pointer flex items-center gap-1.5 text-sm hover:underline truncate"
                                            title="Копіювати"
                                        >
                                            <Mail size={14} className="shrink-0"/> <span className="truncate">{selectedSubmission.form_data.email}</span>
                                        </div>
                                    </div>
                                    <div className="bg-(--platform-card-bg) p-4 rounded-lg border border-(--platform-border-color) shadow-sm">
                                        <div className="text-xs uppercase text-(--platform-text-secondary) font-bold mb-2.5 flex items-center gap-1.5">
                                            <Check size={14} className="shrink-0"/> Статус
                                        </div>
                                ...
                                <div className="bg-(--platform-card-bg) p-5 lg:p-6 rounded-lg border border-(--platform-border-color) min-h-50 shadow-sm">
                                    <div className="text-xs uppercase text-(--platform-text-secondary) font-bold mb-3 flex items-center gap-1.5">
                                        <MessageSquare size={14}/> Повідомлення
                                    </div>
                                    <div className="whitespace-pre-wrap leading-relaxed text-(--platform-text-primary) text-sm lg:text-base wrap-break-word">
                        ...
                        <div className="flex flex-col items-center justify-center h-full text-[#a0aec0] p-4 text-center">
                            ...
                            <div className="text-lg">Оберіть заявку зі списку ліворуч</div>
                        </div>
                    )}
                </div>
            </div>
