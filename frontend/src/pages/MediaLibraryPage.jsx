...
const MediaLibraryPage = () => {
    ...
    return (
        <div style={{ padding: '2rem', maxWidth: '1200px', margin: 'auto' }}>
            ...
            {error && <div className="bg-danger-light text-danger" style={{ padding: '1rem', borderRadius: '8px', marginBottom: '1rem' }}>{error}</div>}
                ...
                {selectedFile && (
                    <div className="card" style={{ position: 'sticky', top: '2rem' }}>
                        ...
                        <div style={{ marginTop: '1rem' }}>
                            ...
                            <input type="text" readOnly value={`${API_URL}${selectedFile.path_full}`} onFocus={(e) => e.target.select()} />
                            <button className="btn btn-secondary" style={{width: '100%', marginTop: '0.5rem'}} onClick={() => handleCopyToClipboard(`${API_URL}${selectedFile.path_full}`)}>
                                Скопіювати URL
                            </button>
                        </div>
                    </div>
